...
async def dispand(message):
    ...
    for m in messages:
        ...
        if m.content or m.attachments:
            sent_message = await message.channel.reply(embed=compose_embed(m))
        ...
        for attachment in m.attachments[1:]:
            ...
            sent_attachment_message = await message.channel.reply(embed=embed)
            sent_messages.append(sent_attachment_message)

        for embed in m.embeds:
            sent_embed_message = await message.channel.reply(embed=embed)
