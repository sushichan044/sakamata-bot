...
class MemberConfView(View):
    ...
    async def ok(self, interaction: discord.Interaction):
        ...
        self.left_button = Button(self.ng_str).style(self.ng_style).disabled(self.status is False).on_click(self.ng).url(self.ng_url)
        self.right_button = Button(self.ok_str).style(discord.ButtonStyle.green).disabled(self.status is not None).on_click(self.ok)
