...
class SongDBHttpClient:
    ...
    async def search_by_song(self, song_name: str) -> Any:
        return await self.request(endpoint=f"?title={song_name}")

    async def search_by_artist(self, artist_name: str) -> Any:
        return await self.request(endpoint=f"?artist={artist_name}")

    async def search_by_stream(self, stream_id: str) -> Any:
        return await self.request(endpoint=f"?stream={stream_id}")

    async def search_no_recent_song(self) -> Any:
